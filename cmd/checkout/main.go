...
func main() {
	...
	slog.Info("Checkout Service", "version", version, "commit", commit, "date", date)
	...
	server := &http.Server{
		Addr:           ":8080",
		Handler:        utils.LogMiddleware(router.EnableCorsHeader(utils.TracingMiddleware("checkout")(mux))),
